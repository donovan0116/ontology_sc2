...
- 状态黑板；
- Economy、Construction、Production、Technology、Scout、Combat Manager；
- 统一命令调度器；
- 资源预算、任务生命周期和失败监视；
- 更完整、可配置的规则 `TacticalAdvisor`；
- 保持现有 `GameSnapshot -> MacroIntent` 边界兼容。

V0.2 应优先实现统一命令调度器和资源预算，再逐个加入 Manager；否则多个 Manager 会
竞争同一资源，难以解释失败。
